...
int main(int argc, char *argv[])
{
    ...
    if (me == 0)
    {
        ...
        while (fin_count < send_count)
        {
            ...
            if (send_count < Dwidth / Nrect && status.MPI_TAG == FIN_TAG)
            {
                int width = send_count * Nrect;
                MPI_Send(&width, Alloc, MPI_INT, status.MPI_SOURCE, ALLOCATE_TAG, MPI_COMM_WORLD);
                log_array[0][send_count] = MPI_Wtime();
                send_count++;
            }
            ...
            if (total_count % 20 == 0)
                write_ppm(name, total_count, np, Dwidth, Dheight, MandleBuffer);
        }
        ...
        for (int t = 0; t < Dwidth / Nrect; t++)
        {
            ...
            printf("dynamic, process, %d, worker, %d, time, %f\n", np, t, temp);
            time_average += temp;
        }
